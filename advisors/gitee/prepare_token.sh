...
curl -X POST --data-urlencode "grant_type=password" --data-urlencode "username=${email}" --data-urlencode "password=${password}" --data-urlencode "client_id=${client_id}" --data-urlencode "client_secret=${client_secret}" --data-urlencode "scope=projects user_info issues notes" https://gitee.com/oauth/token > ~/.gitee_personal_token.json

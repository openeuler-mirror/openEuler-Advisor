...
# ******************************************************************************/
"""
This is a helper script for working with atomgit.com
"""
...
class Atomgit():
    """
    Atomgit is a helper class to abstract api.atomgit.com api
    """
    _instance_lock = threading.Lock()
    _first_init = True

    def __new__(cls):
        if not hasattr(Atomgit, "_instance"):
            with Atomgit._instance_lock:
                if not hasattr(Atomgit, "_instance"):
                    Atomgit._instance = object.__new__(cls)
        return Atomgit._instance

    def __init__(self):
        if not self._first_init:
            return
        self._first_init = False
        with open(os.path.expanduser("~/.atomgit_personal_token.json"), "r") as secret:
            self.token = json.load(secret)

        self.headers = {'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW 64; rv:50.0) '\
                        'Gecko/20100101 Firefox/50.0'}
        self.community_url = "https://api.atomgit.com/api/v5/repos/openeuler/community/contents/"
        self.src_openeuler_url = "https://api.atomgit.com/api/v5/repos/src-openeuler/{repo}/contents/"
        self.advisor_url = "https://api.atomgit.com/api/v5/repos/openeuler/openEuler-Advisor/contents/"

        self.helper_info = {}
        specfile_exception_url = self.advisor_url + "advisors/helper/specfile_exceptions.yaml"
        resp = self.__get_json(specfile_exception_url)
        if not resp:
        ...
        resp = self.__get_json(version_exception_url)
        if not resp:
        ...
        resp = self.__get_json(upgrade_branches_url)
        if not resp:
        ...
        resp = self.__get_json(reviewer_checklist_url)
        if not resp:
    ...
    def __post_atomgit(self, url, values, headers=None):
        """
        POST into atomgit API
        """
        if headers is None:
    ...
    def __patch_atomgit(self, url, values, headers=None):
        """
        PATCH method to atomgit API
        """
        if headers is None:
            headers = self.headers.copy()
        data = urllib.parse.urlencode(values).encode('utf-8')
    ...
    def fork_repo(self, repo, owner="src-openeuler"):
        """
        Fork repository in atomgit 
        """
        url_template = "https://api.atomgit.com/api/v5/repos/{owner}/{repo}/forks"
        ...
        #headers["HOST"] = "atomgit.com"
        #headers["Accept"] = "*/*"
        return self.__post_atomgit(url, values)

    def create_issue(self, repo, version, branch):
        """
        Create issue in atomgit
        """
    ...
    def get_reviewers(self, repo, owner="src-openeuler"):
        ...
        """
        url_template = "https://api.atomgit.com/api/v5/repos/{owner}/{pkg}/collaborators"
        url = url_template.format(owner=owner, pkg=repo)
        return self.__get_atomgit(url)

    def list_pr(self, repo, owner="src-openeuler"):
        """
        List all prs of repo
        """
        url_template = "https://api.atomgit.com/api/v5/repos/{owner}/{repo}/pulls?state=open&per_page=100&page=1"
        url = url_template.format(owner=owner, repo=repo)
        return self.__get_json(url)

    def create_pr(self, repo, version, branch, owner="src-openeuler"):
        """
        Create PR in atomgit
        """
        assignees = ""
        ...
        url_template = "https://api.atomgit.com/api/v5/repos/{owner}/{pkg}/pulls"
        url = url_template.format(owner=owner, pkg=repo)
        values = {}
        values["access_token"] = self.token["access_token"]
    ...
    def create_pr_comment(self, repo, number, body, owner="src-openeuler"):
        ...
        """
        url_template = "https://api.atomgit.com/api/v5/repos/{owner}/{repo}/pulls/{number}/comments"
    ...
    def edit_pr_comment(self, owner, repo, comment_id, body):
        ...
        edit a comment
        """
        url_template = "https://api.atomgit.com/api/v5/repos/{owner}/{repo}/pulls/comments/{comment_id}"
        url = url_template.format(owner=owner, repo=repo, comment_id=comment_id)
        values = {}
        values["access_token"] = self.token["access_token"]
        values["body"] = body
        return self.__patch_atomgit(url, values)

    def __get_atomgit(self, url, headers=None):
        """
        GET from atomgit api
        """
        if '?' in url:
            new_url = url + "&"
        else:
        ...
        try:
            ...
            print("get_atomgit failed to access: %s" % (url))
            print("get_atomgit failed: %d, %s" % (error.code, error.reason))
            return None
        except urllib.error.URLError as error:
            print("get_atomgit failed to access: %s"  % (url))
            print("get_atomgit failed: %s" % (error.reason))

    def get_pr(self, repo, num, owner="src-openeuler"):
        """
        Get detailed information of the given specific PR
        """
        url_template = "https://api.atomgit.com/api/v5/repos/{owner}/{repo}/pulls/{number}"
    ...
    def get_diff(self, repo, num, owner="src-openeuler"):
        ...
        """
        url_template = "https://atomgit.com/{owner}/{repo}/pull/{number}.diff"
        url = url_template.format(owner=owner, repo=repo, number=num)
        resp = self.__get_atomgit(url)
        return resp
    ...
    def get_sig_info(self, sig):
        ...
        """
        url_template = "https://raw.atomgit.com/openeuler/community/raw/master/sig/{sig}/sig-info.yaml"
        url = url_template.format(sig=sig)
        #req = urllib.request.Request(url=url, headers=self.headers)
        resp = self.__get_atomgit(url, self.headers)
        return resp
    ...
    def __get_json(self, url):
        """
        Get and load atomgit json response
        """
        json_resp = []
        headers = self.headers.copy()
        headers["Content-Type"] = "application/json;charset=UTF-8"
        resp = self.__get_atomgit(url, headers)
        if resp:
    ...
    def __get_community_tree(self, sha):
        ...
        """
        page = 1
        url_template = "https://api.atomgit.com/api/v5/repos/openeuler/community/git/trees/{sha}?page={page}"
        url = url_template.format(sha=sha,page=page)
        trees = []
        while True:
            dirs = self.__get_json(url)
            if dirs['tree'] == []:
                return trees
            else:
                trees += dirs['tree']
    ...
    def get_issues(self, pkg, group="src-openeuler"):
        ...
        """
        issues_url = "https://api.atomgit.com/api/v5/repos/{owner}/{repo}/issues?"\
        ...
        for i in range(1, 101):
            ...
            issues = self.__get_json(issues_url + parameters)
            if not issues:
                break
    ...
    def get_issue_comments(self, pkg, number, group="src-openeuler"):
        ...
        """
        issues_url = "https://api.atomgit.com/api/v5/repos/{owner}/{repo}/issues/{number}/comments?"\
                .format(owner=group, repo=pkg, number=number)
        param_template = "page={}&per_page=100&order=asc"
        result = []
        for i in range(1, 101):
            parameters = param_template.format(i)
            comments = self.__get_json(issues_url + parameters)
            if not comments:
                break
    ...
    def post_issue(self, pkg, title, body, prj="src-openeuler"):
        ...
        Post new issue
        """
        issues_url = "https://api.atomgit.com/api/v5/repos/{prj}/issues".format(prj=prj)
        parameters = {}
        parameters["access_token"] = self.token["access_token"]
        parameters["repo"] = pkg
        parameters["title"] = title
        parameters["body"] = body
        self.__post_atomgit(issues_url, parameters)

    def post_issue_comment(self, pkg, number, comment, prj="src-openeuler"):
        """
        Post comment of issue
        """
        issues_url = "https://api.atomgit.com/api/v5/repos/{prj}/{pkg}/issues/{number}/"\
    ...
    @staticmethod
    def get_atomgit_datetime(time_string):
        """
        Get datetime of atomgit
        """
